        ...
        <CardContent>
          <Stack direction={{ xs: "column", md: "row" }} spacing={3} sx={{ justifyContent: "space-between", alignItems: { md: "center" } }}>
            <Box>
              ...
              <Typography color="text.secondary">Gateway oficial Meta Graph API para o ERP</Typography>
              <Typography className="mt-2" color={status?.connected ? "success.main" : "error.main"} sx={{ fontWeight: 700 }}>
                {status?.connected ? "Conectado" : "Desconectado"}
              </Typography>
              <Typography className="mt-1" color="text.secondary" variant="body2">
                Provider: {status?.provider ?? "meta"}
              </Typography>
              <Typography className="mt-1" color="text.secondary" variant="body2">
                Phone: {status?.phone ?? "indisponivel"}
              </Typography>
              ...
              {status?.nextRetryAt && !status.connected && (
            ...
            <Stack direction="row" spacing={2}>
              <Button variant="contained" onClick={connect} disabled={loading}>
                {loading ? <CircularProgress size={20} /> : "Validar credenciais"}
              </Button>
            </Stack>
          </Stack>
        </CardContent>
